...
void Mixture::computeEnergyScores(const GSHOTPyramid & pyramid, vector<Tensor3D> & scores,
                                  ...
                                  vector<vector<vector<Model::Positions> > > * positions) const
{
    ...
    vector<vector<Tensor3D> > convolutions;
...
#pragma omp critical
    ...
    vector<vector<Tensor3D> > convolutions(filterCache_.size());
...
#pragma omp parallel for
	for (int i = 0; i < nbModels; ++i) {
        vector<vector<Tensor3D> > tmp(models_[i].parts().size());
